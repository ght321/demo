...
export function applyArchiveBossState(archive) {
    ...
    if (typeof archive.currentBoss !== 'undefined') {
        ...
        // 恢复Boss血量（defense/health）到UI
        if (currentBoss) {
            const bossHealth = document.getElementById('boss-health');
            if (bossHealth) {
                // 优先用currentBoss.health，否则用defense
                bossHealth.textContent = typeof currentBoss.health !== 'undefined'
                    ? currentBoss.health
                    : (typeof currentBoss.defense !== 'undefined' ? currentBoss.defense : '0');
            }
            const bossAttack = document.getElementById('boss-attack');
            if (bossAttack) bossAttack.textContent = currentBoss.attack || '0';
            // 同步currentBoss.defense = health，保证下次存档一致
            if (typeof currentBoss.health !== 'undefined') {
                currentBoss.defense = currentBoss.health;
            } else if (typeof currentBoss.defense !== 'undefined') {
                currentBoss.health = currentBoss.defense;
            }
        }
    }
